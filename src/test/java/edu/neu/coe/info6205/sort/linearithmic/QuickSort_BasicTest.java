...
public class QuickSort_BasicTest {
    ...
    public void testSort() throws Exception {
        ...
        xs[2] = 2;
        xs[3] = 1;
        Sort<Integer> s = new QuickSort_Basic<>(xs.length, Config.load(getClass()).setupConfig("true", "false", "0", "1", "1", ""));
        Integer[] ys = s.sort(xs);
        assertEquals(Integer.valueOf(1), ys[0]);
        assertEquals(Integer.valueOf(2), ys[1]);
        assertEquals(Integer.valueOf(3), ys[2]);
    ...
    public void testSortWithInstrumenting0() throws Exception {
        ...
        final InstrumentedComparableHelper<Integer> helper = (InstrumentedComparableHelper<Integer>) sorter.getHelper();
        ...
        assertEquals(71, helper.getHits());
    ...
    public void testSortWithInstrumenting5() throws Exception {
        ...
        final Integer[] sorted = sorter.sort(xs);
        assertTrue(helper.isSorted(sorted));
        assertEquals(12377L, helper.getCompares());
        assertEquals(2221L, helper.getSwaps());
        assertEquals(13371L, helper.getHits());
    ...
    public void testSortWithInstrumenting5a() throws Exception {
        ...
        final Integer[] sorted = sorter.sort(xs);
        assertTrue(helper.isSorted(sorted));
        assertEquals(12166L, helper.getCompares());
        assertEquals(2614L, helper.getSwaps());
        assertEquals(12762L, helper.getHits());
    ...
    public void testPartition3() throws Exception {
        ...
        for (int i = 0; i < array.length; i++) array[i] = charArray[i];
        final Config config = Config.setupConfig("true", "false", "0", "1", "", "");
        final NonComparableHelper<Character> helper = HelperFactory.create("quick sort", array.length, config);
        ...
        assertEquals(compares + 2, (int) statPack.getStatistics(InstrumentedComparableHelper.HITS).mean());
    }
...
//        assertTrue(compares <= worstCompares); // FIXME sometimes this fails.
    ...
    public static void beforeClass() throws IOException {
        config_instr = Config.setupConfig("true", "false", "3", "0", "1", "");
    }
